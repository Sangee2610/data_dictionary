...
from app.models.datas import DataSet, Tag, DataSourceOwner, TagValue, DatasetView, DataSetFavorite, DatasetAttribute, DataSetAttachment, TagTest
from flask import request, jsonify, abort, g, send_file, redirect
...
from flask_appbuilder.fieldwidgets import Select2ManyWidget		# Ishi
from wtforms.ext.sqlalchemy.fields import QuerySelectMultipleField  # Ishi


# Ishi
def tag_query():
	db.session.execute("""TRUNCATE TABLE tag_test;""")
	db.session.execute("""
	INSERT INTO tag_test (tag_name ) 
	SELECT concat(t.tag_name,' : ',tv.tag_value_name) 
	FROM tag_values tvs 
	JOIN tag t ON t.tag_id = tvs.tag_id 
	JOIN tag_value tv  ON tv.tag_value_id = tvs.tag_value_id;""")
	return db.session.query(TagTest)

def createdon(date=None):
...
class DataSetView(ModelView):
	...
	'personally_identifiable_information', 'data_source_owner', 'tags_value',
	...
	add_form_extra_fields = {
        #"tags_value": QuerySelectMultipleField(
        #"Tags"
	...
	def list_new(self):
		...
		output= [{'dataset_name': result.dataset_name,\
		     ...
		     'noofdays': createdon(result.created_on),
	...
	@has_access
	def approve(self, pk):
		dataset = db.session.query(DataSet).filter_by(dataset_id=pk).first()
	...
	def post_add_redirect(self):
		...
		dataset_id = db.session.query(DataSet).order_by(DataSet.dataset_id.desc()).first()
